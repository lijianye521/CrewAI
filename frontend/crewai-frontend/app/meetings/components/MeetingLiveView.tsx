...
import { UserOutlined, PlayCircleOutlined, PauseCircleOutlined } from '@ant-design/icons';
...
export default function MeetingLiveView({ meetingId }: MeetingLiveViewProps) {
  ...
  const startMeetingConversation = async () => {
    try {
      ...
      if (response.ok) {
        ...
        message.error(`启动对话失败: ${errorData.detail}`);
      }
    } catch (error) {
      message.error('网络错误');
    }
  ...
  const pauseMeetingConversation = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        message.error(`暂停对话失败: ${errorData.detail}`);
      }
    } catch (error) {
      message.error('网络错误');
    }
          ...
          <Card title="会议统计" size="small">
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span>参与者:</span>
                <span>{meeting?.participants_count || agents.length}人</span>
              </div>
              <div className="flex justify-between">
                <span>消息数:</span>
